...
class Follower:
	...
	# 	x,y = center
	# 	lower_color = numpy.array([15,16,153])
	# 	upper_color = numpy.array([17,18,155])
	...
	def is_red(self, image, center, color = (154,17,16)):
		cx, cy = center
		cy -= 80
		return self.is_color(image, (cx,cy), color)

	def is_blue(self, image, center, color = (0,0,159)):
		cx, cy = center
		cy -= 40
		return self.is_color(image, (cx, cy), color)

	def is_green(self, image, center, color = (39,159,39)):
		cx, cy = center
		cy -= 140
		return self.is_color(image, (cx, cy), color)
	...
	def image_callback(self, msg):
		if self.state == "FOLLOW":
			# converting ROS messages into the OpenCV image format
			image = self.bridge.imgmsg_to_cv2(msg,desired_encoding='bgr8') 
			
			# converting between RGB representation to its equivalent HSV space
			hsv = cv2.cvtColor(image, cv2.COLOR_BGR2HSV) 
			
			# creating lower and upper bounds for the desired hues in the HSV space
			...
			if M['m00'] > 0:
				...
				# P-controller -- a linear scaling of an error drives the control output
				self.twist.linear.x = 0.2
